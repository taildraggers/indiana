"""Aggregates Indiana-located listings from every taildraggers.com
...
ones whose location ends in ", IN".
...
Illinois repos' approach, with the state filter swapped to "IN".
"""
...
PAGE_TITLE = "Taildragger Ads in Indiana"
STATE_ABBR = "IN"
...
_INDIANA_LOCATION_RE = re.compile(r",\s*IN\s*$", re.IGNORECASE)


def _is_indiana(location: str) -> bool:
    return bool(_INDIANA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        indiana_rows = [row for row in rows if _is_indiana(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(indiana_rows)} in Indiana")
        all_listings.extend(indiana_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Indiana listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Indiana listings: {len(listings)}")
