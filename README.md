...
Daily aggregator of Indiana-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "IN".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, IN` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Indiana"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/indiana/`
...
```html
<iframe
  src="https://taildraggers.github.io/indiana/"
  title="Taildragger Ads in Indiana"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_INDIANA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
